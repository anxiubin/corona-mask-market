		...
		<>
			<section className="map-modal">
				...
				<div className="modal-contents">
					...
					<p>
						* 코로나마스크마켓에서 제공하는 공적마스크 판매정보는{" "}
						<strong>5분 이상 지연된</strong> 정보입니다.
					</p>
					<p>
						* 일선에서 수고하시는 약사님 등 현장에서 힘써주시는 모든 분들께
						감사의 마음을 전합니다.
					</p>
					<p style={{ color: "red" }}>
						* 실제 약국에 있는 재고량과 오차가 발생할 수 있으니 절대로 현장에
						계신 약사님들께 문제를 제기하지 말아주세요.
					</p>
				</div>
				<div className="modal-footer">
					<div>데이터 관련 문의: 한국정보화진흥원(maskdata@nia.or.kr)</div>
					<button className="btn-agree" onClick={onClick_agree}>
						동의합니다
					</button>
				</div>
			</section>
			...
			<section className="date-modal">
				...
				<h2 className="day">
					{!localStorage.getItem("LS_birth")
						? "정보없음"
						: localStorage.getItem("LS_birth")}
				</h2>
				<p className="day-ref">[토,일 : 주중에 못산 경우 구매 가능]</p>
				<div className="modal-hidden">
					<div className="birth">출생연도</div>
					...
					<input
						...
						maxLength="4"
					/>
					<button className="birth-submit" onClick={onClick_submit}>
						제출
					</button>
